...
GdkPixbuf *
gdk_pixbuf_new_from_file_at_scale_wic (const gchar *filename, gint width,
                                       gint height, gboolean keep, GError **pe)
{
  GdkPixbuf *pixbuf;
  char buffer[0x1000];
  ...
  if (!SUCCEEDED (hr))
    {
      if (pe)
        {
          *pe = g_error_new (g_quark_from_string ("Warning"), hr,
                             "Create WICImagingFactory failed: %x: %s", hr,
                             strerror (hr));
        }
  ...
  hr = IWICImagingFactory_CreateDecoderFromFilename (
      m_pIWICFactory, wfilename, NULL, GENERIC_READ,
  ...
  hr = IWICBitmapScaler_Initialize (pIScaler,
                                    (IWICBitmapSource *)pIDecoderFrame, width,
                                    height, WICBitmapInterpolationModeCubic);
  if (!SUCCEEDED (hr))
    {
      if (pe)
        {
  ...
  IWICBitmapFrameDecode_CopyPixels (
      pIDecoderFrame, rect, GetStride (width, 32), sizeof buffer, buffer);

  pixbuf = gdk_pixbuf_new_from_data ((const guchar *)buffer,
                                     GDK_COLORSPACE_RGB, FALSE, 8, width,
                                     height, width * 4, NULL, pe);
